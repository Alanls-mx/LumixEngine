...
const liveUpdateMs = 3600;

export function EcosystemVisual({ type, isActive }: EcosystemVisualProps) {
  switch (type) {
    case 'crm': return <CrmMock isActive={isActive} />;
    case 'scheduling': return <SchedulingMock isActive={isActive} />;
    case 'commerce': return <CommerceMock isActive={isActive} />;
    case 'payments': return <PaymentsMock isActive={isActive} />;
    case 'capture':
    default: return <CaptureMock isActive={isActive} />;
  }
}
      ...
      <div className="grid min-h-[34rem] grid-cols-1 sm:grid-cols-[8.5rem_minmax(0,1fr)]">
        <aside className="hidden border-r border-slate-800 bg-[#0a1019] p-3 sm:flex sm:flex-col">
          ...
          <nav className="space-y-1" aria-label="Navegação do sistema demonstrativo">
            {nav.map((item, index) => (
              <div className={`flex items-center gap-2 rounded-lg px-2 py-2 text-xs font-semibold ${index === 1 ? 'bg-emerald-400/10 text-emerald-200' : 'text-slate-500'}`} key={item.label}>
          ...
          <div className="mt-auto border-t border-slate-800 pt-4">
            <div className="flex items-center gap-2 px-2 text-xs font-semibold text-slate-500"><Settings2 className="h-4 w-4" aria-hidden="true" />Configurações</div>
          </div>
        </aside>
        ...
        <main className="min-w-0 overflow-hidden bg-[#0d1420]">
          <div className="flex min-h-16 flex-wrap items-center justify-between gap-3 border-b border-slate-800 px-4 py-3">
            <div><p className="text-xs font-semibold text-slate-500">{module}</p><h4 className="text-sm font-extrabold text-white sm:text-base">{section}</h4></div>
            <div className="flex items-center gap-2">
...
function Metric({ label, value, detail, accent = false }: MetricProps) {
  return (
    <div className="min-w-0 border-r border-slate-800 px-3 py-1 first:pl-0 last:border-r-0 last:pr-0 sm:px-4">
      ...
      <p className="mt-0.5 hidden truncate text-xs text-slate-500 md:block">{detail}</p>
    </div>
  );
}
    ...
    <ProductFrame module="Captação" icon={BarChart3} section="Origem e distribuição de oportunidades">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="min-w-0 rounded-xl border border-slate-800 bg-[#101925] p-4">
          <div className="flex items-center justify-between"><div><p className="text-sm font-bold text-white">Entradas por canal</p><p className="mt-1 text-xs text-slate-500">Registros recebidos e identificados</p></div><Status tone="cyan">Tempo real</Status></div>
          <div className="mt-6 flex h-36 items-end gap-2 sm:gap-3">
            {[38, 54, 47, 72, 61, 84, 76, 92, 68, 88, 96, 82].map((height, index) => (
              <motion.div className="min-w-0 flex-1 rounded-t bg-emerald-400/20" animate={{ height: `${step === index % 3 ? Math.min(height + 8, 100) : height}%` }} transition={{ duration: 0.5, ease: 'easeOut' }} key={`${height}-${index}`}><div className="h-1 rounded-t bg-emerald-400" /></motion.div>
            ))}
          </div>
          <div className="mt-3 flex justify-between text-xs font-semibold text-slate-600"><span>Site</span><span>Campanhas</span><span>Indicação</span><span>WhatsApp</span></div>
        </div>
        <div className="min-w-0 rounded-xl border border-slate-800 bg-[#101925] p-4">
          <p className="text-sm font-bold text-white">Fluxo do último lead</p>
          <div className="mt-4 space-y-1">
            {events.map(([title, detail, time], index) => (
              <div className="relative flex gap-3 pb-4 last:pb-0" key={title}>
                {index < events.length - 1 ? <span className="absolute left-3 top-6 h-full w-px bg-slate-700" /> : null}
                <motion.span className={`relative z-10 mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border ${index <= step ? 'border-emerald-400 bg-emerald-400 text-slate-950' : 'border-slate-700 bg-slate-900 text-slate-500'}`} animate={{ scale: index === step ? [1, 1.12, 1] : 1 }}><Check className="h-3.5 w-3.5" aria-hidden="true" /></motion.span>
                <div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><p className="truncate text-xs font-bold text-slate-100">{title}</p><span className="shrink-0 text-xs text-slate-600">{time}</span></div><p className="mt-1 truncate text-xs text-slate-500">{detail}</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-slate-800 bg-[#101925]">
        <div className="grid grid-cols-[1.2fr_0.8fr_0.8fr] border-b border-slate-800 px-4 py-2 text-xs font-semibold text-slate-500"><span>Oportunidade</span><span>Origem</span><span>Status</span></div>
...
function CrmMock({ isActive }: { isActive: boolean }) {
  const step = useLiveStep(isActive, 3, 4200);
  const columns = [
  ...
  return (
    <ProductFrame module="Gestão comercial" icon={Users} section="Pipeline de oportunidades">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <div className="relative min-w-0 flex-1 sm:max-w-xs"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" aria-hidden="true" /><div className="h-9 rounded-lg border border-slate-700 bg-slate-950/50 pl-9 pr-3 text-xs leading-9 text-slate-500">Buscar cliente ou negócio</div></div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {columns.map((column, columnIndex) => (
          <div className="min-w-0 rounded-xl border border-slate-800 bg-[#101925] p-3" key={column.title}>
            <div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-200">{column.title}</p><span className="text-xs tabular-nums text-slate-500">{column.count}</span></div>
            <div className="mt-3 space-y-2">
              {column.cards.map(([name, value, owner], cardIndex) => {
                const highlighted = columnIndex === step && cardIndex === 0;
                return (
                  <motion.div className={`rounded-lg border bg-[#0c1420] p-3 ${highlighted ? 'border-emerald-400/60' : 'border-slate-800'}`} animate={{ y: highlighted ? -3 : 0, boxShadow: highlighted ? '0 12px 30px rgba(16,185,129,0.10)' : '0 0 0 rgba(0,0,0,0)' }} transition={{ duration: 0.45, ease: 'easeOut' }} key={name}>
                    <div className="flex items-start justify-between gap-2"><p className="truncate text-xs font-bold text-white">{name}</p><MoreHorizontal className="h-4 w-4 shrink-0 text-slate-600" aria-hidden="true" /></div>
                    <p className="mt-2 text-sm font-extrabold tabular-nums text-slate-200">{value}</p>
                    <div className="mt-3 flex items-center justify-between"><span className="flex items-center gap-1 text-xs text-slate-500"><Clock3 className="h-3.5 w-3.5" />Hoje, 15:30</span><Avatar initials={owner} color={owner === 'LS' ? 'bg-emerald-600' : 'bg-slate-600'} /></div>
                  </motion.div>
                );
              })}
            </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-slate-800 bg-[#101925] px-4 py-3"><div className="min-w-0"><p className="truncate text-xs font-bold text-white">Próxima ação: revisar escopo do Grupo Ventura</p><p className="mt-1 truncate text-xs text-slate-500">Histórico, anexos e conversa disponíveis no registro</p></div><Status tone="amber">Hoje</Status></div>
    </ProductFrame>
  );
}
    ...
    <ProductFrame module="Agenda" icon={CalendarDays} section="Semana de atendimento">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4">
        <div className="flex items-center gap-2">
          ...
          <div><p className="text-xs font-bold text-white">Dra. Beatriz Santos</p><p className="text-xs text-slate-500">Odontologia</p></div>
        </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_12rem]">
        <div className="min-w-0 overflow-hidden rounded-xl border border-slate-800 bg-[#101925]">
          <div className="grid grid-cols-5 border-b border-slate-800">
            {days.map((day) => <div className="border-r border-slate-800 px-1 py-2 text-center text-xs font-semibold text-slate-500 last:border-r-0 sm:px-2" key={day}>{day}</div>)}
          </div>
                ...
                {[0, 1, 2, 3].map((row) => {
                  ...
                  return (
                    <div className="h-20 border-b border-slate-800 p-1 last:border-b-0 sm:p-1.5" key={row}>
                      {booking ? (
                        <motion.div className={`h-full overflow-hidden rounded-lg border p-1.5 sm:p-2 ${booking.color}`} animate={{ opacity: step >= bookingIndex ? 1 : 0.55, scale: step === bookingIndex ? [0.98, 1.02, 1] : 1 }} transition={{ duration: 0.45 }}>
        ...
        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
          <div className="rounded-xl border border-slate-800 bg-[#101925] p-3">
            <p className="text-xs font-semibold text-slate-500">Ocupação da semana</p><p className="mt-1 text-xl font-extrabold text-white">78%</p>
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-800"><motion.div className="h-full bg-emerald-400" animate={{ width: isActive ? '78%' : '0%' }} transition={{ duration: 0.8 }} /></div>
          </div>
          <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-3">
            <MessageSquareText className="h-4 w-4 text-emerald-300" aria-hidden="true" /><p className="mt-3 text-xs font-bold text-emerald-100">Confirmação enviada</p><p className="mt-1 text-xs leading-5 text-emerald-200/70">Gabriel confirmou o horário das 09:00.</p>
          </div>
          <div className="rounded-xl border border-slate-800 bg-[#101925] p-3">
            <p className="text-xs font-bold text-white">Próximo lembrete</p><p className="mt-1 text-xs text-slate-500">Hoje, 18:00</p><div className="mt-3"><Status tone="cyan">24h antes</Status></div>
          </div>
        </div>
      </div>
    </ProductFrame>
...
function CommerceMock({ isActive }: { isActive: boolean }) {
  ...
  return (
    <ProductFrame module="Comércio" icon={Store} section="Pedidos e catálogo">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="min-w-0 overflow-hidden rounded-xl border border-slate-800 bg-[#101925]">
          <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3"><p className="text-sm font-bold text-white">Catálogo publicado</p><div className="flex items-center gap-2 text-xs font-semibold text-slate-500"><Store className="h-4 w-4" />Loja + canais</div></div>
          <div className="divide-y divide-slate-800">
            {[
              ...
              <div className="grid grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-3" key={sku}>
                ...
                <div className="min-w-0"><p className="truncate text-xs font-bold text-white">{name}</p><p className="mt-1 text-xs text-slate-500">{sku} · {stock}</p></div>
        ...
        <div className="rounded-xl border border-slate-800 bg-[#101925] p-4">
          <div className="flex items-center justify-between"><p className="text-sm font-bold text-white">Pedido #1842</p><Status>Em andamento</Status></div>
          <div className="mt-4 flex items-center gap-3"><Avatar initials="CM" color="bg-cyan-700" /><div><p className="text-xs font-bold text-white">Camila Martins</p><p className="text-xs text-slate-500">2 itens · R$ 3.380,00</p></div></div>
          <div className="mt-5 space-y-3">
            {stages.map((stage, index) => (
              <div className="flex items-center gap-3" key={stage}>
                <motion.div className={`flex h-6 w-6 items-center justify-center rounded-full border ${index <= step ? 'border-emerald-400 bg-emerald-400 text-slate-950' : 'border-slate-700 text-slate-600'}`} animate={{ scale: index === step ? [1, 1.14, 1] : 1 }}><Check className="h-3.5 w-3.5" /></motion.div>
                <span className={`text-xs font-semibold ${index <= step ? 'text-slate-100' : 'text-slate-600'}`}>{stage}</span>
              </div>
            ))}
          </div>
      ...
      <AnimatePresence mode="wait" initial={false}>
        <motion.div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3" key={step} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
          <div className="min-w-0"><p className="truncate text-xs font-bold text-emerald-100">{stages[step]}</p><p className="mt-1 truncate text-xs text-emerald-200/70">Estoque, cliente e canal atualizados no mesmo registro.</p></div><CheckCircle2 className="h-5 w-5 shrink-0 text-emerald-300" />
        </motion.div>
      </AnimatePresence>
    </ProductFrame>
  );
  ...
  return (
    <ProductFrame module="Financeiro" icon={CreditCard} section="Cobranças e conciliação">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[0.82fr_1.18fr]">
        <div className="rounded-xl border border-slate-800 bg-[#101925] p-4">
          <div className="flex items-center justify-between"><p className="text-sm font-bold text-white">Cobrança #PX-2048</p><Status tone={paid ? 'green' : 'amber'}>{paid ? 'Pix recebido' : 'Aguardando'}</Status></div>
          ...
          <div className="mt-4 border-t border-slate-800 pt-4"><div className="flex justify-between text-xs"><span className="text-slate-500">Valor</span><span className="font-extrabold text-white">R$ 250,00</span></div><div className="mt-2 flex justify-between text-xs"><span className="text-slate-500">Vínculo</span><span className="font-semibold text-slate-300">Reserva #804</span></div></div>
        </div>
        <div className="min-w-0 rounded-xl border border-slate-800 bg-[#101925] p-4">
          <div className="flex items-center justify-between"><p className="text-sm font-bold text-white">Linha do pagamento</p><Webhook className="h-4 w-4 text-slate-500" /></div>
          <div className="mt-5 space-y-1">
            {events.map((event, index) => (
              <div className="relative flex gap-3 pb-5 last:pb-0" key={event.title}>
                {index < 2 ? <span className={`absolute left-4 top-8 h-full w-px ${index < step ? 'bg-emerald-400/50' : 'bg-slate-700'}`} /> : null}
                <motion.div className={`relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${index <= step ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-300' : 'border-slate-700 bg-slate-900 text-slate-600'}`} animate={{ scale: index === step ? [1, 1.08, 1] : 1 }}><event.icon className="h-4 w-4" /></motion.div>
                <div className="min-w-0"><p className={`text-xs font-bold ${index <= step ? 'text-white' : 'text-slate-600'}`}>{event.title}</p><p className="mt-1 truncate text-xs text-slate-500">{event.detail}</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-slate-800 bg-[#101925]">
        <div className="grid grid-cols-[1fr_0.7fr_0.8fr] border-b border-slate-800 px-4 py-2 text-xs font-semibold text-slate-500"><span>Cliente</span><span>Valor</span><span>Status</span></div>
        <div className="grid grid-cols-[1fr_0.7fr_0.8fr] items-center px-4 py-3 text-xs"><span className="truncate font-bold text-white">Juliana Paes</span><span className="tabular-nums text-slate-300">R$ 250,00</span><Status>{reconciled ? 'Conciliado' : paid ? 'Confirmando' : 'Pendente'}</Status></div>
      </div>
    </ProductFrame>
  );
}
